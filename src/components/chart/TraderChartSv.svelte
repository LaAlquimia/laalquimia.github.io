  ...
  <div class="max-w-7xl mx-auto flex flex-col gap-6 sm:gap-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/5 pb-6">
      <div>
        <div class="flex items-center gap-2.5 flex-wrap">
          <span class="px-2.5 py-0.5 text-[11px] font-semibold tracking-wider text-cyan-400 bg-cyan-950/40 border border-cyan-800/50 rounded-full">QUANT V2 FÚTURES</span>
          <span class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
          <span class="text-xs text-gray-400">Bybit Conexión Activa</span>
        </div>
        <h1 class="text-2xl sm:text-3xl md:text-4xl font-extrabold tracking-tight mt-2 bg-gradient-to-r from-white via-slate-200 to-indigo-400 bg-clip-text text-transparent">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="lg:col-span-2 flex flex-col gap-6">
        <div class="glass-card p-3.5 sm:p-6 flex flex-col gap-4">
          <div class="flex flex-wrap justify-between items-center gap-2">
            <div class="flex items-center gap-2.5">
              <span class="text-base sm:text-lg font-bold text-gray-200">{selectedSymbol}</span>
        ...
        <!-- Scanner / Watchlist -->
        <div class="glass-card p-3.5 sm:p-6 flex flex-col gap-4 relative overflow-hidden">
          {#if isScanning}
          ...
          <div class="flex justify-between items-center gap-2">
            <div>
              <h3 class="text-base sm:text-lg font-bold text-gray-200 m-0">Scanner Multiactivo</h3>
              <span class="text-[11px] text-gray-500 sm:hidden">Desliza para ver columnas</span>
            </div>
            <button class="px-3 py-1.5 sm:px-4 text-xs font-semibold text-white bg-indigo-600 hover:bg-indigo-500 active:scale-95 transition-all rounded-lg glow-violet whitespace-nowrap" on:click={triggerScan} disabled={isScanning}>
              {isScanning ? `(${scanProgress}/${scanTotal})...` : "Escanear"}
            </button>
          </div>
          <div class="overflow-x-auto rounded-xl">
            <table class="gtable min-w-[540px]">
              <thead>
                <tr>
                  <th class="text-left">Symbol</th>
                  <th class="text-right">Precio</th>
                  <th class="text-right">Macro</th>
      ...
      <div class="flex flex-col gap-6">
        ...
        <!-- Checklist Card -->
        <div class="glass-card p-3.5 sm:p-6 flex flex-col gap-5 border-l-4 border-cyan-500">
          <div>
            <h3 class="text-base sm:text-lg font-bold text-gray-200 m-0">Gatillo Institucional (5M)</h3>
            <p class="text-xs text-gray-400 mt-0.5">Condiciones de entrada al cierre de vela.</p>
          </div>
          ...
          <!-- Final Signal Box -->
          <div class="p-3.5 rounded-xl text-center border font-bold text-xs sm:text-sm {isLongTriggered ? 'bg-emerald-950/60 border-emerald-500 text-emerald-400 glow-green' : 'bg-white/5 border-white/10 text-gray-400'}">
        ...
        <div class="glass-card p-3.5 sm:p-6 flex flex-col gap-4">
          <h3 class="text-base sm:text-lg font-bold text-gray-200 m-0">Métricas Cuánticas (5M)</h3>
        ...
        <div class="glass-card p-3.5 sm:p-6 flex flex-col gap-3">
          <h3 class="text-base sm:text-lg font-bold text-gray-200 m-0">Alertas Recientes</h3>
          <div class="flex flex-col gap-2 max-h-40 overflow-y-auto">
            ...
            {:else}
              <p class="text-xs text-gray-500 text-center py-3">Esperando señales...</p>
            {/each}
          </div>
        </div>
